...
export default function EstimateDialog({ isOpen, onClose, selectedTrees, onSuccess, defaultClientId }: EstimateDialogProps) {
    ...
    async function generateEstimateNumber(supabase: ReturnType<typeof createClient>): Promise<string> {
        ...
        const lastNum = parseInt(data[0].estimate_number.replace('E-', ''), 10)
        return `E-${String(lastNum + 1).padStart(5, '0')}`
    }
    ...
    async function handleSubmit() {
        ...
        try {
            const estimateNumber = await generateEstimateNumber(supabase)
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200">
                <div className="p-6 border-b border-gray-100">
                    <h2 className="text-xl font-bold text-gray-800">見積もり作成</h2>
                    <p className="text-sm text-gray-500 mt-1">{selectedTrees.length} 本の樹木で見積もりを作成します</p>
                </div>
                ...
                <div className="p-6 space-y-4 max-h-[60vh] overflow-y-auto">
                    ...
                    <div>
                        ...
                        <textarea
                            ...
                            placeholder="見積条件や補足事項など"
                        />
                    </div>

                    {/* 選択ツリー確認 */}
                    <div className="bg-gray-50 rounded-xl p-4">
                        <p className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">見積対象明細</p>
                                    ...
                                    <li key={tree.id} className="flex justify-between items-center">
                                        ...
                                        {rate < 1 ? (
                                            <span className="font-mono text-right">
                                                <span className="text-gray-400 line-through text-xs mr-1">&yen;{tree.price.toLocaleString()}</span>
                                                <span>&yen;{unitPrice.toLocaleString()}</span>
                                            </span>
                                        ) : (
                                            <span className="font-mono">&yen;{tree.price.toLocaleString()}</span>
                                        )}
                                    </li>
                                )
                            })}
                        ...
                        <div className="border-t border-gray-200 mt-2 pt-2 flex justify-between font-bold text-gray-800">
                            <span>合計金額</span>
                            <span>&yen;{selectedTrees.reduce((sum, t) => sum + Math.round(t.price * rate), 0).toLocaleString()}</span>
                        </div>
                    </div>
                </div>
                ...
                <div className="p-6 bg-gray-50 flex gap-3">
                    ...
                    <button
                        ...
                        className="flex-[2] bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-3 rounded-xl font-bold shadow-lg shadow-emerald-200 transition-all active:scale-95 disabled:opacity-50"
                        disabled={loading || !selectedClientId}
                    >
                        {loading ? '作成中...' : '見積もりを作成する'}
                    </button>
                </div>
            </div>
        </div>
