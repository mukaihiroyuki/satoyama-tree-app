...
export default function ShipmentPortalPage({ params }: { params: Promise<{ clientId: string; shipmentId: string }> }) {
    const { clientId, shipmentId } = React.use(params)
    ...
    async function fetchData() {
        ...
        setReceiptCompleted((shipment as unknown as { receipt_completed_at: string | null }).receipt_completed_at)

        // 受入チェック済みのアイテムを取得
        const itemIds = (shipment.shipment_items || []).map((i: { id: string }) => i.id)
    ...
    function downloadCSV() {
        const headers = ['管理番号', '樹種', '樹高(m)', '本立ち', ...(client?.portal_show_price ? ['単価(円)'] : []), '受入', '備考']
        ...
        a.href = url
        a.download = `${client?.name || 'trees'}_${shippedAt}_納品リスト.csv`
        a.click()
            ...
            <header className="bg-emerald-700 text-white px-4 py-6">
                <div className="max-w-3xl mx-auto">
                    <div className="flex items-center gap-2 mb-1">
                        <Link href={`/c/${clientId}`} className="text-emerald-200 text-sm hover:text-white">
                            ← 出荷一覧
                        </Link>
                    </div>
                    ...
                    <p className="text-emerald-200 text-sm mt-1">
                        出荷日: {shippedAt} / {totalCount} 本 / {receivedCount} 本受入済み
                    </p>
                </div>
            </header>
