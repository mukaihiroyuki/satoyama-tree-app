...
## 📍 現在地 (2026-06-11 更新)
- **出荷済み 年度別表示**: トップに「📦 出荷済み（年度別）」カードを追加（R7/R8…を本数表示、押すと該当年度の出荷済み一覧へ）。樹木一覧にも「出荷年度」フィルター追加。年度=4月〜翌3月（令和N年度=西暦2018+N年4/1〜）。年度計算は `src/lib/fiscal-year.ts` に集約しトップ/一覧で共用。**重要: 出荷日は trees に無く shipments が正本** → `shipment_items(shipments(shipped_at))` をJOINして取得（tree-repositoryと同方式）。集計は **Supabase 1000件制限を避けて `.range()` で全件ページング**（年度別合計=出荷済み総数2444に一致を確認）。
...
## 🔜 次回やるべきこと
- **【最優先・本丸】🔴 圏外で「初見の木」が開けない致命欠陥の修正**: 圏外で一覧/QRから「その端末で未訪の木」をタップすると `ERR_FAILED` で行き止まり（SWの pages キャッシュが `maxEntries:32` だが木は5540本／データはIndexedDBに全件ある）。**fix方針は確定済み・実装のみ未着手**: Service Workerで `/trees/*` の圏外フォールバックに「全木共通の汎用詳細シェル(app shell)」を返す。シェルはURL(pathname)からUUIDを読みIndexedDBから描画。**印刷済み物理QR(`/trees/{uuid}`)を壊さないようURL構造は維持**（URL変更案Bは再印刷リスクで却下済み）。検証は ローカルWindowsビルド不通のため ブランチ→Vercelプレビュー→圏外テスト(Chrome DevTools Offline)で 🔴→✅ を実証してから本番。詳細は memory `project_offline_unseen_tree_gap`。
...
## 💬 申し送り
2026-06-25: **アカシデ採番NaNバグを根本修正・本番デプロイ（commit 87ad64b, Vercel Ready確認済み）。** 事務員FB「アカシデをPC手入力しても正常な番号が採番されない」が発端。ダッシュボードの「訳アリ34本」を全件クロスチェック→大多数は誤登録/取り違いの正規退役記録（壊れた木ではない）と判明。唯一の異物 `26-AD-0NaN` の根因は `syncPendingRegistrations` の採番が**文字列降順の先頭採用**だったこと（誤登録修正フローの「仮」番号 `26-AD-仮0001` が数字より後ろにソート→`parseInt('仮0001')=NaN`→`0NaN`、さらに一意制約23505で2本目以降のアカシデ登録が無音破棄され端末滞留）。修正＝`registerTreeOffline` と同じ「isNaN弾く数値最大」方式に統一・`limit(1000)`付き。**全25樹種＋空＋AO(3664本)境界を実データ全件検証してからデプロイ**。併せてダッシュボードの赤「訳アリ」バッジを撤去→管理メニュー「無効化済み」グレー移設。`26-AD-0NaN` レコードは事務員確認で実物なし＝無効放置で決着。詳細 memory `project_acashide_numbering_nan`。**3大課題の現状: ①QR-UUID不一致=原因特定済み・対応済み(運用徹底のみ)、②圏外で初見の木=fix方針確定・未着手→次回本丸、③E2E=保険・最後でOK。** 棚上げ案: 無効化の理由構造化設計(`project_disable_reason_design`, Mukai判断待ち)。

2026-06-11: 社長依頼（小笠原さん経由）で「出荷済みを年度別に表示」を実装・本番リリース（commit bd0bba4 / 型修正 784f9c8、Vercel Ready）。年度=4月〜翌3月。結果 R7=1614本 / R8=830本（合計2444＝出荷済み総数と一致）。作業は自宅PC（`C:\Users\hiroy\satoyama-tree-app`）。**踏んだ落とし穴2つ**: ①出荷日は trees に無く shipments が正本（ShipmentDialogは status だけ更新）→JOINで取得。②Supabaseの1000件制限で年度別合計が一旦1000で頭打ち→`.range()`全件ページングで解消。後者は半年前にも踏んだ再発案件で、メモリ `feedback_supabase_1000_row_limit` に記録（新規selectは毎回ページング/count/RPCを意識）。**未処理**: `git stash@{0}`（前回自宅で作った `docs/design/` 設計書3本＋CLAUDE.md追記）が退避したまま宙ぶらりん。会社PCには PROJECT_CONTEXT.md が別途あり。戻す/捨てるを次回判断。
